...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Initialize audio settings - CRITICAL for autoplay
    audio.muted = false;
    audio.volume = 0.5;
    audio.preload = 'auto';
    let musicStarted = false;
    ...
    // Function to safely play audio with multiple retry strategies
        ...
        try {
            ...
            if (playPromise !== undefined) {
                playPromise.then(() => {
                    ...
                    console.warn("Play attempt 1 failed:", error1.message);
                    
                    // Attempt 2: Retry with small delay
                    setTimeout(() => {
                        audio.play().then(() => {
                            musicStarted = true;
                            updateButtonIcon();
                            console.log("✓ Music playing (retry)");
                        }).catch(error2 => {
                            console.warn("Play attempt 2 failed:", error2.message);
                            
                            // Attempt 3: Force unmuted and try again
                            setTimeout(() => {
                                audio.muted = false;
                                audio.play().catch(e => console.error("Final play failed:", e.message));
                            }, 200);
                        });
                    }, 100);
                });
    ...
    const toggleMusic = () => {
        if (isMuted()) {
            audio.muted = false;
            // When unmuting, ensure the music starts
            if (audio.paused) {
                playAudio();
            }
        } else {
    ...
    // 2. Play Music on Entrance Button Click (HIGH PRIORITY)
    if (entranceButton) {
        entranceButton.addEventListener('click', playAudio);
